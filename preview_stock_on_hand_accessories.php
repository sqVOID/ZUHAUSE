...
if ($is_superadmin || ($session_branch !== '' && strtoupper($session_branch) !== 'SUPERADMIN')) {
    ...
    if ($group_f !== '')
        $conds[] = "group_name = '" . addslashes($group_f) . "'";
    if ($dept_f !== '')
        $conds[] = "department = '" . addslashes($dept_f) . "'";
    if ($family_f !== '')
        $conds[] = "family_code = '" . addslashes($family_f) . "'";
    $where = implode(" AND ", $conds);

    $sql = "SELECT item_code, description, SUM(quantity) AS total_qty, branch, family_code
...
$pdf->SetY(23);
$pdf->Cell(0, 10, 'STOCK ON HAND - ACCESSORIES', 0, 1, 'C');
$pdf->Ln(3);
...
if ($group_f) {
    $pdf->Cell(135, 6, 'GROUP: ' . $group_f, 0, 1, 'L');
}
if ($dept_f) {
    $pdf->Cell(135, 6, 'DEPARTMENT: ' . $dept_f, 0, 1, 'L');
}
if ($family_f) {
...
// Output PDF
$filename = 'Stock_On_Hand_Accessories_' . date('Y-m-d') . '.pdf';
