...
// RESTRICT ACCESS: Only Sub-admin can access this page
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="item-selection-container">
            ...
            <div class="footer-actions"
                ...
                <div class="footer-left-group">
                    <button type="button" class="btn-new" onclick="createNewEntry()">NEW</button>
                ...
                <div class="footer-right-group">
                    <!-- <button type="button" class="btn-skip-receipt" onclick="skipCancelReceipt()">CANCEL INVOICE NO</button> GLOBALLY HIDDEN -->
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn-back-modal"
                        onclick="closeUnclaimedFreebieSearchModal()">Back</button>
                </div>
            </div>
        </div>
        ...
        <script>
            // Global flag to track if NEW button has been clicked
            ...
            function createNewEntry() {
                // CHECK IF DATE IS FILLED FIRST
                const dateField = document.getElementById('date');
                if (!dateField.value || dateField.value.trim() === '') {
                    alert('Please select a Date first before clicking NEW!');
                    dateField.focus();
                    return; // Stop execution if date is not filled
                }

                // Set the flag to indicate NEW button has been clicked
                isNewButtonClicked = true;

                // Store the current invoice number and date before resetting
                const currentInvoiceNo = document.getElementById('invoice_no').value;
                const currentDate = dateField.value;

                // 1. Reset Main Form Inputs (but we'll preserve invoice_no and date)
                ...
                // 5. Reset Discount Field to locked state
                ...
                // 6. Clear IMEI Field (keep it enabled for user input)
                ...
                // Clear invoice number field for manual input and make it editable
                const invoiceNoField = document.getElementById('invoice_no');
                invoiceNoField.value = '';
                invoiceNoField.removeAttribute('readonly');
                invoiceNoField.style.backgroundColor = 'white';
                invoiceNoField.style.cursor = 'text';

                // Preserve the date value and lock it after clicking NEW
                // Restore the date value that was saved before the reset
                dateField.value = currentDate;
                // Lock the date field so it can't be changed
            ...
            function saveSalesEntry() {
                ...
                if (items.length === 0) {
                    alert('Please add at least one item to the late entry!');
                    return;
                }
                ...
                // Prepare data object
                const data = {
                    invoice_no: document.getElementById('invoice_no').value.trim(), // Include custom invoice number
                    ...
                    date: document.getElementById('date').value, // Include the selected date
                ...
                fetch('save_sales_entry.php', {
                    ...
                    .then(result => {
                        ...
                        if (result.status === 'success') {
                            alert('Late entry saved successfully!\nInvoice No: ' + result.invoice_no);
                    ...
                    .catch(error => {
                        ...
                        saveBtn.disabled = false;
                        console.error('Error:', error);
                        alert('An error occurred while saving the late entry. Please try again.');
                    });
            }
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Attach event listener to Add Unclaimed Freebies button
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // Set max date to yesterday (prevent today and future dates)
                const dateField = document.getElementById('date');
                if (dateField) {
                    ...
                    // Format to YYYY-MM-DD for the max attribute
                    ...
                    for (let row of existingRows) {
                        ...
                        if (cells.length >= 2) {
                            ...
                            if (existingSerial === imei.trim()) {
                                alert('This IMEI (' + imei + ') has already been added to this late entry!');
            ...
            addButton.addEventListener('click', function () {
                ...
                if (!dateField || dateField === '') {
                    alert("Please select Date before adding items!");
                ...
                if (!invoiceNo || invoiceNo.trim() === '') {
                    alert("Please input Invoice Number before adding items!");
                ...
                function proceedWithAddingItem() {
                    ...
                    `;
                            ...
                            newRow.setAttribute('data-base-price', price); // Store base price for promo calculations

                            const itemPricesStr = itemCodeInput.getAttribute('data-prices') || (typeof selectedItemPrices !== 'undefined' ? JSON.stringify(selectedItemPrices) : '{}');
                            const itemOthersBankStr = itemCodeInput.getAttribute('data-others-bank-enabled') || (selectedItemOthersBankEnabled ? '1' : '0');
                            newRow.setAttribute('data-prices', itemPricesStr);
                            newRow.setAttribute('data-others-bank-enabled', itemOthersBankStr);
            ...
            window.updateTotals = function () {
                ...
                if (pointsField) {
                    ...
                    // Change background color based on whether points is enabled
                    if (hasAnyPoints) {
