...
$group_f = isset($_GET['group']) ? trim($_GET['group']) : '';
$dept_f = isset($_GET['department']) ? trim($_GET['department']) : '';
...
if ($has_full_access) {
    ...
    if ($branch_f !== '' || $model_f !== '' || $brand_f !== '' || $family_f !== '' || $group_f !== '' || $dept_f !== '' || $search !== '') {
        $show_initial_message = false;
        $conds = ["s.item_type = 'Accessories'"];
        if ($search !== '')
        ...
        if ($group_f !== '')
            $conds[] = "s.group_name = '" . addslashes($group_f) . "'";
        if ($dept_f !== '')
            $conds[] = "s.department = '" . addslashes($dept_f) . "'";
        if ($family_f !== '')
            $conds[] = "s.family_code = '" . addslashes($family_f) . "'";
        $where = implode(" AND ", $conds);

        $sql = "SELECT s.item_code, s.description, SUM(s.quantity) AS total_qty, s.branch, s.family_code,
                   COALESCE(i.brand, s.brand, '—') AS brand, s.status
            FROM stock_on_hand s
            LEFT JOIN items i ON s.item_code = i.item_code
            WHERE $where
            GROUP BY s.item_code, s.description, s.branch, s.family_code, i.brand, s.brand, s.status 
            HAVING total_qty > 0
    ...
    // Filter directly on stock_on_hand columns — handles any combination of empty/filled filters
    // Only run query if at least one filter is applied
    if ($branch_f !== '' || $model_f !== '' || $brand_f !== '' || $family_f !== '' || $group_f !== '' || $dept_f !== '' || $search !== '') {
        $show_initial_message = false;
        $conds = ["s.item_type = 'Accessories'", "s.branch IN ($branch_in)"];
        if ($search !== '')
            $conds[] = "(s.item_code LIKE '%" . addslashes($search) . "%' OR s.description LIKE '%" . addslashes($search) . "%')";
        if ($status_f !== '')
            $conds[] = "s.status = '" . addslashes($status_f) . "'";
...
<head>
    ...
    <title>Stock On Hand - Accessories</title>
...
<body>
    ...
    <div class="main-content">
        <div class="content-header">
            ...
            <div class="button-group">
                ...
                <a href="sohandaccessories.php" class="btn-summary active">Stock Summary Accessories</a>
            </div>
        </div>

        <form method="GET" action="sohandaccessories.php"
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                ...
                <div>
                    ...
                    <select name="model" class="searchable-select"
                        ...
                        <?php
                        $model_res = $conn->query("SELECT DISTINCT item_code FROM stock_on_hand WHERE item_type = 'Accessories' ORDER BY item_code");
                ...
                <div>
                    <label style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Group:</label>
                    <select name="group" class="searchable-select"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                        <option value="">Groups</option>
                        <?php
                        $group_res = $conn->query("SELECT id, group_name FROM `groups` WHERE status = 'Active' ORDER BY group_name");
                        if ($group_res) {
                            while ($g = $group_res->fetch_assoc()):
                                ?>
                                <option value="<?php echo htmlspecialchars($g['group_name']); ?>" <?php echo $group_f === $g['group_name'] ? 'selected' : ''; ?>>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                <div>
                    <label
                        style="display: block; margin-bottom: 8px; font-weight: 600; color: #333;">Department:</label>
                    <select name="department" class="searchable-select"
                        style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px;">
                        <option value="">Departments</option>
                        <?php
                        $dept_res = $conn->query("SELECT id, department_name FROM departments WHERE status = 'Active' ORDER BY department_name");
                        if ($dept_res) {
                            while ($d = $dept_res->fetch_assoc()):
                                ?>
                                <option value="<?php echo htmlspecialchars($d['department_name']); ?>" <?php echo $dept_f === $d['department_name'] ? 'selected' : ''; ?>>
        ...
        <div class="table-container">
            ...
            <h3>Stock Summary Accessories</h3>
    ...
    <script>
        ...
        function printReport() {
            ...
            const group = '<?php echo addslashes($group_f); ?>';
            const department = '<?php echo addslashes($dept_f); ?>';
            const family = '<?php echo addslashes($family_f); ?>';

            // Build URL with parameters
            let url = 'preview_stock_on_hand_accessories.php?';
            const params = [];
        ...
        function exportToExcel() {
            ...
            const groupLabel = group || 'All Groups';
            ...
            const headers = ['Model Code', 'Description', 'Qty', 'Branch', 'Family', 'Status'];
            ...
            ws['!cols'] = [
                ...
                { wch: Math.round(35 * EXPORT_SCALE) }, // Description
            ...
            headers.forEach((h, ci) => {
                const addr = XLSX.utils.encode_cell({ r: 8, c: ci });
                cs(addr, {
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Stock Accessories');
            const fileName = 'Stock_On_Hand_Accessories_' + (branch || 'All').replace(/\s+/g, '_') + '_' + new Date().toISOString().slice(0, 10) + '.xlsx';
