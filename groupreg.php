...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register_group'])) {
    ...
    if ($group_name_raw !== trim($group_name_raw)) {
        $message = "Group name contains special characters! (Leading or trailing spaces are not allowed)";
        $messageType = "error";
    }
    // Check for special characters (only allow alphanumeric and spaces)
    else if (!preg_match('/^[A-Za-z0-9 ]+$/', $group_name_raw)) {
        $message = "Group name contains special characters! (Only letters, numbers, and spaces are allowed)";
        $messageType = "error";
    }
    else {
        $group_name = strtoupper($conn->real_escape_string($group_name_raw));

        // Check if updating or inserting
        if (isset($_POST['group_id']) && !empty($_POST['group_id'])) {
        // Update existing group
        $id = $conn->real_escape_string($_POST['group_id']);
        
        // Check if group name already exists (excluding current record)
        $check_sql = "SELECT id FROM `groups` WHERE group_name='$group_name' AND id != $id";
        $check_result = $conn->query($check_sql);
        
        if ($check_result && $check_result->num_rows > 0) {
            $message = "Group name '$group_name' already exists!";
            $messageType = "error";
        } else {
            $sql = "UPDATE `groups` SET group_name='$group_name' WHERE id=$id";

            if ($conn->query($sql) === TRUE) {
                $redirect_url = "groupreg.php?updated=1";
        ...
        // Check if group already exists before inserting
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            // Insert new group
            $sql = "INSERT INTO `groups` (group_name, status) VALUES ('$group_name', 'Active')";

            if ($conn->query($sql) === TRUE) {
                $redirect_url = "groupreg.php?success=1";
                if (!empty($selected_filter)) {
                    $redirect_url .= "&status_filter=" . urlencode($selected_filter);
                }
                header("Location: $redirect_url");
...
if (isset($_GET['status_update'])) {
    ...
    $sql = "UPDATE `groups` SET status = '$new_status' WHERE id = $id";

    if ($conn->query($sql) === TRUE) {
        $redirect_url = "groupreg.php";
        if (!empty($selected_filter)) {
            $redirect_url .= "?status_filter=" . urlencode($selected_filter);
        }
        header("Location: $redirect_url");
...
// Fetch all groups based on selected filter
...
if (!empty($selected_filter)) {
    ...
    $groups_result = $conn->query($query);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="main-content">
        <div class="content-header">
            ...
            <button class="btn-add-group" onclick="toggleForm()">Add Group</button>
        </div>
        ...
        <div id="formContainer" class="form-container <?php echo $editMode ? '' : 'hidden'; ?>">
            <form method="POST" action="">
                <?php if ($editMode): ?>
                    <input type="hidden" name="group_id" value="<?php echo $editData['id']; ?>">
                    <?php
                endif; ?>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="group_name" required
                            value="<?php echo $editMode ? $editData['group_name'] : ''; ?>"
                            placeholder="Enter group name" oninput="this.value = this.value.toUpperCase()">
                    </div>
                </div>

                <div class="form-actions">
                    <button type="button" class="btn-cancel" onclick="toggleForm()">Cancel</button>
                    <button type="submit" name="register_group" class="btn-save">
                    ...
                    if ($groups_result === null) {
                        ...
                        echo "<tr><td colspan='3' style='text-align: center; padding: 30px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc;'>
                                <div style='display: flex; flex-direction: column; align-items: center; gap: 12px;'>
                                    ...
                                    <div style='color: #666; font-size: 13px;'>Please select a status filter from the dropdown above to view groups.</div>
                                </div>
                              </td></tr>";
                    } elseif ($groups_result && $groups_result->num_rows > 0) {
                        while ($row = $groups_result->fetch_assoc()) {
                            ...
                            echo "</td>";
                            echo "<td>";
                            // Build edit URL with filter parameter
                            $edit_url = "groupreg.php?edit=" . $row['id'];
                            if (!empty($selected_filter)) {
                            ...
                            echo "</button>";
                            echo "<a href='groupreg.php?delete=" . $row['id'] . "' class='btn-delete' ";
                            echo "onclick='return confirm(\"Are you sure you want to delete this group?\")'>Delete</a>";
                            echo "</td>";
                            echo "</tr>";
                        }
                    } else {
                        // No results for the selected filter
                        echo "<tr><td colspan='3' style='text-align: center !important; padding: 40px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc; color: #666;'>No groups found for the selected filter</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function filterByStatus() {
            ...
            if (statusFilter) {
                window.location.href = 'groupreg.php?status_filter=' + encodeURIComponent(statusFilter);
            } else {
                window.location.href = 'groupreg.php';
            }
        }
        ...
        function searchTable() {
            ...
            <?php if ($groups_result !== null): ?>
            for (let i = 1; i < tr.length; i++) {
                const tds = tr[i].getElementsByTagName('td');
                if (!tds || tds.length === 0) continue;

                // Skip the "SELECT A FILTER" or "No data found" message rows
                if (tds[0] && tds[0].getAttribute('colspan')) {
                    continue;
                }

                // Search group name
                const groupName = tds[0] ? (tds[0].textContent || tds[0].innerText) : '';
                const searchMatch = groupName.toLowerCase().indexOf(searchInput) > -1;
            ...
            if (visibleCount === 0 && searchInput.trim() !== '') {
                ...
                noDataRow.innerHTML = '<td colspan="3" style="text-align: center !important; padding: 30px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc; color: #666; font-size: 14px;">No data found matching your search.</td>';
        ...
        function toggleStatus(id, currentStatus) {
            ...
            let url = `groupreg.php?status_update=1&id=${id}&status=${statusParam}`;
            if (statusFilter) {
