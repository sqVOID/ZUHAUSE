...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register_supplier'])) {
    ...
    if (isset($_POST['supplier_id']) && !empty($_POST['supplier_id'])) {
        ...
        if ($conn->query($sql) === TRUE) {
            $redirect_url = "supplierreg.php?updated=1";
        ...
        // Insert new supplier
        $sql = "INSERT INTO suppliers (store_name, contact_number, address, status) 
                VALUES ('$store_name', '$contact_number', '$address', 'Active')";

        if ($conn->query($sql) === TRUE) {
            $redirect_url = "supplierreg.php?success=1";
            if (!empty($selected_filter)) {
                $redirect_url .= "&status_filter=" . urlencode($selected_filter);
            }
            header("Location: $redirect_url");
...
if (isset($_GET['activate'])) {
    ...
    $sql = "UPDATE suppliers SET status='Active' WHERE id = $id";

    if ($conn->query($sql) === TRUE) {
        $message = "Supplier activated successfully!";
        $messageType = "success";
        $redirect_url = "supplierreg.php?activated=1";
        if (!empty($selected_filter)) {
            $redirect_url .= "&status_filter=" . urlencode($selected_filter);
        }
        header("Location: $redirect_url");
...
// Fetch all suppliers based on selected filter
...
<body>
    ...
    <div class="main-content">
        <div class="content-header">
            ...
            <button type="button" class="btn-add-supplier" onclick="toggleForm()">Add Supplier Registration</button>
        </div>
        ...
        <form method="POST" action="" class="form-container hidden" id="supplierForm">
            <input type="hidden" name="supplier_id" id="supplier_id"
                value="<?php echo $editMode && $editData ? $editData['id'] : ''; ?>">
            <div class="form-row">
                <div class="form-group">
                    <label>Supplier Name</label>
                    <input type="text" name="store_name" id="store_name" placeholder="Enter supplier name"
            ...
            <div class="form-actions">
                ...
                <button type="submit" name="register_supplier"
        ...
        <div class="table-container">
            <div class="table-header">
                ...
                <div style="display: flex; gap: 15px; align-items: center;">
                    <select id="statusFilter" onchange="filterByStatus()" style="padding: 8px 12px; border: 1px solid #ddd; border-radius: 4px; font-size: 14px; cursor: pointer;">
                        ...
                        <option value="Inactive" <?php echo ($selected_filter === 'Inactive') ? 'selected' : ''; ?>>Inactive</option>
                    </select>
            ...
            <table id="supplierTable">
                ...
                <tbody>
                    <?php
                    if ($suppliers_result === null) {
                        // No filter selected - show message
                        echo "<tr><td colspan='5' style='text-align: center; padding: 30px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc;'>
                                <div style='display: flex; flex-direction: column; align-items: center; gap: 12px;'>
                                    ...
                                    <div style='color: #666; font-size: 13px;'>Please select a status filter from the dropdown above to view suppliers.</div>
                                </div>
                              </td></tr>";
                    } elseif ($suppliers_result && $suppliers_result->num_rows > 0) {
                        while ($row = $suppliers_result->fetch_assoc()) {
                            ...
                            echo "<td>";
                            // Preserve status_filter when editing, activating, deactivating, or deleting
                            ...
                            echo "<a href='" . $edit_url . "' class='btn-edit' onclick='return editSupplier(" . $row['id'] . ")'>Edit</a>";
                            if ($status !== 'Active') {
                                echo "<a href='" . $activate_url . "' class='btn-activate' onclick='return confirm(\"Are you sure you want to activate this supplier?\")'>Activate</a>";
                            }
                            if ($status == 'Active') {
                                echo "<a href='" . $deactivate_url . "' class='btn-deactivate' onclick='return confirm(\"Are you sure you want to deactivate this supplier?\")'>Deactivate</a>";
                            }
                            echo "<a href='" . $delete_url . "' class='btn-delete' onclick='return confirm(\"Are you sure you want to delete this supplier?\")'>Delete</a>";
                            echo "</td>";
                            echo "</tr>";
                        }
                    } else {
                        // No results for the selected filter
                        echo "<tr><td colspan='5' style='text-align: center !important; padding: 40px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc; color: #666;'>No suppliers found for the selected filter</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function toggleForm() {
            ...
            const button = document.querySelector('.btn-add-supplier');
            ...
            if (form.classList.contains('hidden')) {
                ...
                if (urlParams.has('edit')) {
                    ...
                    let redirectUrl = 'supplierreg.php';
     ...
     <?php if ($editMode && $editData): ?>
        ...
        function filterByStatus() {
            ...
            if (statusFilter) {
                window.location.href = 'supplierreg.php?status_filter=' + encodeURIComponent(statusFilter);
            } else {
                window.location.href = 'supplierreg.php';
            }
        }
        ...
        function searchTable() {
            ...
            <?php if ($suppliers_result !== null): ?>
            for (let i = 1; i < tr.length; i++) {
                const tds = tr[i].getElementsByTagName('td');
                if (!tds || tds.length === 0) continue;

                // Search across supplier name, contact, and address
                const supplierName = tds[0] ? (tds[0].textContent || tds[0].innerText) : '';
                const contact = tds[1] ? (tds[1].textContent || tds[1].innerText) : '';
                const address = tds[2] ? (tds[2].textContent || tds[2].innerText) : '';
                const combinedText = (supplierName + ' ' + contact + ' ' + address).toLowerCase();
                
                const searchMatch = combinedText.indexOf(searchInput) > -1;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            searchInputElement.placeholder = 'Search suppliers...';
