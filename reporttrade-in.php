...
<head>
    ...
    <title>Trade-In Sales Report</title>
    <style>
        :root {
        ...
        * {
        ...
        /* -- TRADE-IN REPORT WRAPPER -- */
        .tradein-report-wrapper {
            background: #fff;
...
<body>
    ...
    <div class="main-content" id="mainContent">
        <div class="page-title">Trade-In Sales Report</div>
        ...
        <div class="tradein-report-wrapper">
            <div class="tradein-report-header">Trade-In Sales Report</div>
            <div class="receipt-area">
                <div class="doc-print-zone">
                    <div class="doc-card">
                        <div class="doc-head">
                            <img src="Icon/ZUHAUSE-LOGO.png" alt="ZUHAUSE Logo" class="doc-logo">
                            <div class="doc-tradein-title">TRADE-IN SALES REPORT</div>
                        </div>
                        <div class="doc-meta">
                            ...
                            <table class="doc-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Branch</th>
                                        <th>Customer Name</th>
                                        <th class="wrap-header">IMEI</th>
                                        <th class="wrap-header">Trade-In Brand</th>
                                        <th class="wrap-header">Trade-In Item</th>
                                        <th class="wrap-header">Trade-In IMEI</th>
                                        <th class="wrap-header">Trade-In Value</th>
                                ...
                                <tbody id="tradeInTableBody">
                                    <tr>
                                        <td colspan="12" class="td-no-data">No trade-in sales data. Click "Search" to load report.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div class="summary-section" id="summarySection" style="display: none;">
                            ...
                            <div class="summary-row">
                                ...
                                <span id="totalTradeInValue">₱0.00</span>
                            </div>
                            <div class="summary-row">
    ...
    <script>
        ...
        function loadTradeInReport() {
            ...
            fetch(`fetch_tradein_report.php?${params.toString()}`)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') {
                        renderTradeInReport(data.data);
                ...
                .catch(error => {
                    ...
                    alert('Error loading trade-in report');
                });
        }

        // Render trade-in report table
        function renderTradeInReport(data) {
            const tbody = document.getElementById('tradeInTableBody');

            if (!data || data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="12" class="td-no-data">No trade-in sales found for the selected criteria.</td></tr>';
            ...
            data.forEach((row, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="td-text-left">${row.sold_imeis || '-'}</td>
                    <td class="td-text-left">${row.tradein_brand || '-'}</td>
                    <td class="td-text-left">${row.tradein_item_code || '-'}</td>
                    <td>${row.tradein_imei || '-'}</td>
                    <td class="td-number">${formatCurrency(row.tradein_value || 0)}</td>
        ...
        function updateSummary(data) {
            ...
            const totalTradeInValue = data.reduce((sum, row) => sum + parseFloat(row.tradein_value || 0), 0);
            const totalSalesAmount = data.reduce((sum, row) => sum + parseFloat(row.total_amount || 0), 0);

            document.getElementById('totalTransactions').textContent = totalTransactions;
            document.getElementById('totalTradeInValue').textContent = formatCurrency(totalTradeInValue);
        ...
        function printReport() {
            ...
            window.open(
                'print_tradein_report_pdf.php?' + params.toString(),
                '_blank',
        ...
        function exportToExcel() {
            ...
            if (!tbody.querySelector('tr:not(.td-no-data)')) {
                alert('No data to export. Please search for trade-in sales first.');
                return;
            }
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'Trade-In Report');

            // Generate filename
            const filename = `TradeIn_Report_${dateFrom}_to_${dateTo}_${area}_${branch}.xlsx`;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            console.log('Trade-In Sales Report page loaded');
        });
    </script>
</body>
