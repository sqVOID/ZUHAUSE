...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['register_department'])) {
    $department_name_raw = $_POST['department_name'];
    
    // Check for leading or trailing spaces
    if ($department_name_raw !== trim($department_name_raw)) {
        $message = "Department name contains special characters! (Leading or trailing spaces are not allowed)";
        $messageType = "error";
    }
    // Check for special characters (only allow alphanumeric and spaces)
    else if (!preg_match('/^[A-Za-z0-9 ]+$/', $department_name_raw)) {
        $message = "Department name contains special characters! (Only letters, numbers, and spaces are allowed)";
        $messageType = "error";
    }
    else {
        $department_name = strtoupper($conn->real_escape_string($department_name_raw));
        ...
        $check_sql = "SELECT id FROM departments WHERE department_name='$department_name' AND id != $id";
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            if ($conn->query($sql) === TRUE) {
                $redirect_url = "departmentreg.php?updated=1";
        ...
        $check_sql = "SELECT id FROM departments WHERE department_name='$department_name'";
        $check_result = $conn->query($check_sql);
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            if ($conn->query($sql) === TRUE) {
                $redirect_url = "departmentreg.php?success=1";
                if (!empty($selected_filter)) {
                    $redirect_url .= "&status_filter=" . urlencode($selected_filter);
                }
                header("Location: $redirect_url");
...
if (isset($_GET['status_update'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        $redirect_url = "departmentreg.php";
        if (!empty($selected_filter)) {
            $redirect_url .= "?status_filter=" . urlencode($selected_filter);
        }
        header("Location: $redirect_url");
...
// Fetch all departments based on selected filter
...
<body>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="department_name" required
                            value="<?php echo $editMode ? $editData['department_name'] : ''; ?>"
                            placeholder="Enter department name" oninput="this.value = this.value.toUpperCase()">
                    </div>
                </div>
                    ...
                    if ($departments_result === null) {
                        ...
                        echo "<tr><td colspan='3' style='text-align: center; padding: 30px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc;'>
                                <div style='display: flex; flex-direction: column; align-items: center; gap: 12px;'>
                                    ...
                                    <div style='color: #666; font-size: 13px;'>Please select a status filter from the dropdown above to view departments.</div>
                                </div>
                              </td></tr>";
                    } elseif ($departments_result && $departments_result->num_rows > 0) {
                        while ($row = $departments_result->fetch_assoc()) {
                            ...
                            $edit_url = "departmentreg.php?edit=" . $row['id'];
                            if (!empty($selected_filter)) {
                            ...
                            echo "</button>";
                            echo "<a href='departmentreg.php?delete=" . $row['id'] . "' class='btn-delete' ";
                            echo "onclick='return confirm(\"Are you sure you want to delete this department?\")'>Delete</a>";
                            echo "</td>";
                            echo "</tr>";
                        }
                    } else {
                        // No results for the selected filter
                        echo "<tr><td colspan='3' style='text-align: center !important; padding: 40px 20px; border-left: 1px solid #ccc; border-right: 1px solid #ccc; border-bottom: 1px solid #ccc; color: #666;'>No departments found for the selected filter</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        function filterByStatus() {
            ...
            if (statusFilter) {
                window.location.href = 'departmentreg.php?status_filter=' + encodeURIComponent(statusFilter);
            } else {
        ...
        function searchTable() {
            ...
            <?php if ($departments_result !== null): ?>
            for (let i = 1; i < tr.length; i++) {
                const tds = tr[i].getElementsByTagName('td');
                if (!tds || tds.length === 0) continue;

                // Search department name
                const deptName = tds[0] ? (tds[0].textContent || tds[0].innerText) : '';
                const searchMatch = deptName.toLowerCase().indexOf(searchInput) > -1;
        ...
        function toggleStatus(id, currentStatus) {
            ...
            let url = `departmentreg.php?status_update=1&id=${id}&status=${statusParam}`;
            if (statusFilter) {
