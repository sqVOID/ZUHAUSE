...
// Parse multiple branch names and their codes
$user_branches = [];
$branch_codes = [];
...
<body>
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="search-bar-wrapper">
            <div class="search-controls">
                <input type="text" id="searchInput" placeholder="Enter ST No." oninput="filterTable(false)">
                ...
                if ($is_super_admin || $is_sub_admin) {
                    ...
                    if ($branch_filter_result && $branch_filter_result->num_rows > 0) {
                        while ($branch_row = $branch_filter_result->fetch_assoc()) {
                            ...
                            $branches_options .= "<option value=\"{$branch_value}\">{$branch_display}</option>";
                        }
                    }

                    echo '<select id="branchFromFilter" onchange="filterTable(false)">';
                    echo '    <option value="">Select Branch (From)</option>';
                    echo '    <option value="ALL">All Branches</option>';
                    echo $branches_options;
                    echo '</select>';

                    echo '<select id="branchToFilter" onchange="filterTable(false)" style="margin-left: 5px;">';
            ...
            <div class="date-filters">
                ...
                <button class="btn-preview-all" onclick="previewAllStockTransfers()">
            ...
            <table class="report-table" id="reportTable">
                <thead>
                    <tr>
                        ...
                        <th style="width:12%;">ST NO.</th>
                        <th style="width:18%;">BRANCH FROM</th>
                        <th style="width:18%;">BRANCH TO</th>
                        <th style="width:9%;">STATUS</th>
                        <th style="width:12%;">PREPARED BY</th>
                        <th style="width:12%;">RECEIVED BY</th>
                        <th style="width:12%;">DISAPPROVED BY</th>
                ...
                <tbody id="reportTableBody">
                    ...
                    <tr class="no-data" style="display: none;">
                        <td colspan="9">No stock transfers found.</td>
                    </tr>
                </tbody>
            </table>
    ...
    <script>
        ...
        function filterTable(isButtonClick = false) {
            // If this is a button click and branch filters exist, validate them
            const branchFromFilterElement = document.getElementById('branchFromFilter');
            const branchToFilterElement = document.getElementById('branchToFilter');
            
            if (isButtonClick && (branchFromFilterElement || branchToFilterElement)) {
                const branchFromFilter = branchFromFilterElement ? branchFromFilterElement.value : '';
                const branchToFilter = branchToFilterElement ? branchToFilterElement.value : '';
                
                // Check if at least one branch is selected (can be either from or to, or both)
                if (!branchFromFilter && !branchToFilter) {
                    alert('Please select at least one branch filter (From or To) before filtering.');
                    if (branchFromFilterElement) branchFromFilterElement.focus();
                    return;
                }
            }
            ...
            rows.forEach(row => {
                ...
                const branchFromCell = row.cells[2];
                const branchToCell = row.cells[3];
                const branchFromText = branchFromCell ? branchFromCell.textContent : '';
                const branchToText = branchToCell ? branchToCell.textContent : '';
                ...
                // Check if row matches branch filters (only if filters exist and are not "ALL")
                let matchesBranchFrom = true;
                let matchesBranchTo = true;

                // Extract branch code. For from it is format "CODE - NAME", for to it is format "CODE - NAME"
                const branchFromMatch = branchFromText.match(/^([A-Z0-9]+) -/);
                const rowBranchFromCode = branchFromMatch ? branchFromMatch[1] : '';

                const branchToMatch = branchToText.match(/^([A-Z0-9]+) -/);
                const rowBranchToCode = branchToMatch ? branchToMatch[1] : '';

                if (branchFromFilter && branchFromFilter !== 'ALL' && rowBranchFromCode) {
                    matchesBranchFrom = (rowBranchFromCode === branchFromFilter);
                }

                if (branchToFilter && branchToFilter !== 'ALL' && rowBranchToCode) {
                    matchesBranchTo = (rowBranchToCode === branchToFilter);
                }
                ...
                const shouldShow = matchesSearch && matchesBranchFrom && matchesBranchTo && matchesDateRange;
        ...
        // Load stock transfer data from server via AJAX
        function loadStockTransferData() {
            ...
            if (branchFromFilter) params.append('branch_from', branchFromFilter);
            if (branchToFilter) params.append('branch_to', branchToFilter);
            if (dateFrom) params.append('date_from', dateFrom);
            if (dateTo) params.append('date_to', dateTo);

            // Show loading indicator
            const tbody = document.getElementById('reportTableBody');
            tbody.innerHTML = '<tr class="no-data"><td colspan="9">Loading data...</td></tr>';

            // Fetch data from server
            fetch('fetch_stock_transfer_data.php?' + params.toString())
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        displayStockTransferData(data.data);
                    } else {
                        tbody.innerHTML = '<tr class="no-data"><td colspan="9">Error loading data: ' + (data.error || 'Unknown error') + '</td></tr>';
                    }
                })
                .catch(error => {
        ...
        // Display stock transfer data in table
        function displayStockTransferData(data) {
            const tbody = document.getElementById('reportTableBody');

            if (!data || data.length === 0) {
                tbody.innerHTML = '<tr class="no-data"><td colspan="9">No stock transfers found.</td></tr>';
                currentPage = 1;
                updatePagination();
                return;
            }
            ...
            data.forEach(row => {
                html += '<tr>';
                html += '  <td>' + row.date + '</td>';
                html += '  <td>' + row.st_number + '</td>';
                html += '  <td>' + row.branch_from + '</td>';
                html += '  <td>' + row.branch_to + '</td>';
                html += '  <td>' + row.status + '</td>';
                html += '  <td>' + row.prepared_by + '</td>';
                html += '  <td>' + row.received_by + '</td>';
                html += '  <td>' + row.disapproved_by + '</td>';
                html += '  <td>';
                html += '      <div class="action-btns">';
                html += '          <button class="btn-preview" onclick="previewStockTransfer(\'' + row.st_number + '\')">Preview</button>';
                html += '      </div>';
                html += '  </td>';
                html += '</tr>';
            });
        ...
        // Preview single stock transfer
        function previewStockTransfer(stNumber) {
            if (stNumber) {
        ...
        // Preview all
        function previewAllStockTransfers() {
            ...
            if (branchFromFilter) {
                params.push(`branch_from=${branchFromFilter}`);
            }
            if (branchToFilter) {
                params.push(`branch_to=${branchToFilter}`);
            }

            const queryString = params.length > 0 ? '?' + params.join('&') : '';
            window.open('preview_all_stock_transfers.php' + queryString, '_blank', 'width=900,height=700');
        }
    </script>
</body>
